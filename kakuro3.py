...
class EnterPuzzle(1):
    ...
    def __init__(self, incoming, middle_x,middle_y):
  
        self.my_edwin = None
        self.from_file = False
        self.mess = 'Please edit data in the white box'
        if incoming != None:
            self.pv = incoming
            self.from_file = True
        else:
            self.pv = SGIO()
        self.expose_ourselves()

    def expose_ourselves(self):
        Toplevel.__init__(self)                  # a new top-level window
        self.title('Enter puzzle')
        self.resizable(False,False)
        base_frame = Frame(self)
        base_frame.grid(row=0, column=0)
        data_frame = Frame(base_frame)
        data_frame.grid(row=0, column=0)
        edwin = Text(data_frame, width=35, height=20, background='white')
        edwin.grid(row=0, column=0)
        self.my_edwin = edwin
        if self.from_file is True:
            text = self.pv.input_lines
            self.my_edwin.insert(END,text)
            bg = 'red'
            self.message_frame = Label(data_frame,text=self.mess,bd=2,bg=bg)
            self.message_frame.grid(row=1, column=0,sticky=EW)
            ok_frame = Frame(data_frame)
            ok_frame.grid(row=2, column=0)

            Button(ok_frame, text='LOAD', \
                       command = self.get_data).grid(row=0, column=0)
            Button(ok_frame, text='QUIT', \
                       command = self.my_destroy).grid(row=0, column=1)
            ...
            Button(ok_frame, text='QUIT', \
                   command = self.my_destroy).grid(row=0, column=1)
        #self.centre_widget(middle_x,middle_y)
        self.grab_set()
        self.focus_set()
        self.wait_window()
    
    def centre_widget(self,middle_x,middle_y):
        self.update_idletasks()
        width = self.winfo_width()
        height = self.winfo_height()
        x = middle_x - int(width/2.0)
        y = middle_y - int(height/2.0)
